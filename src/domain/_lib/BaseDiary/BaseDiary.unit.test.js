...
const closeedItems = [item1, item2, closeItem];
const items = [item1, item2, closeItem, item3];

let coll;
let result;

describe('Domain :: lib :: BaseDiary', () => {
  beforeEach(() => {
    coll = new BaseDiary({ closeValue });
  });
  ...
  context('when passed not close item', () => {
    ...
    context('when items is closeed', () => {
      beforeEach(() => {
        coll.setItems(closeedItems);
      });
