...
const checkSeller = checkEntityTimestamp(
  (seller, mockSeller, { mockPost, mockPostsList, mockSeniorityType }) => {
    if (!seller) {
      return {
    ...
    mockSeller.appointments.forEach(
      ({ postId: { value: postId }, day: { value: day } }, idx) => {
        expect({ postId, day: day.getTime() }).toEqual(
          seller.appointments[idx]
        );
      }
    );
